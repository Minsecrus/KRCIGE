...
自 1968 年 NATO 软件工程会议召开以应对“软件危机”以来，学术界与工业界关于“软件工程”本质内涵的界定经历过数个代表性阶段。Bauer 早期代表 NATO 会议提出经典定义：“建立并使用健全的工程原则，以便经济地获得在实际机器上可靠且能高效运行的软件” \cite{nato1968}，奠定了资源约束与物理可靠性的工程基调；IEEE Std 610.12 国际标准将其规范化为“将系统化、规范化、可度量的方法应用于软件的开发、运行和维护” \cite{ieee1990}，突出流程受控与可度量性；Boehm 在《软件工程经济学》中将其表征为实用经济学、数学与计算科学的结合，聚焦受限资源条件下的工程效用权衡 \cite{boehm1981}；近年 Google 团队则进一步将其凝练为“在时间维度上积分的编程”（\textit{Programming integrated over time}），指出软件工程区别于单纯编程的核心在于跨越时间的系统演化、组织人员规模扩展以及多目标权衡决策 \cite{winters2020}。

上述代表性定义分别从经济性、规范性、度量性与时间演化等视角提供了深刻的工程洞见。然而，现有论述大多停留在宏观的经济愿景、管理学流程规范或生动的工业隐喻层面，长期缺乏一个自底层物理与计算约束出发、能对各类工程经验策略进行形式化公理推导与因果预测的统一数学框架。与此同时，半个多世纪的工程实践中沉淀了浩繁看似分散的最佳实践与法则——从 DRY、低耦合、小步迭代，到版本控制、持续集成、契约断言与防御隔离。一个根本的理论问题在于：这些由不同时代、语言与组织形态提炼出的经验原则，背后是否由更少、更自洽的有界约束与决策机制所支配？

本文提出 KRCIGE 框架，试图把软件工程原则组织为以下自顶向下的形式化推导结构：
\begin{equation}
\boxed{
\text{现实客观约束}
+
...
式 \eqref{eq:trajectory-value} 的长程轨迹价值采用离散时间折现累加形式，这在数学上严格对应了 Google 关于“软件工程是在时间维度上积分的编程”（\textit{Programming integrated over time} \cite{winters2020}）的工程主张。然而，现实中的有限时域折现求和并非平滑无摩擦的数学积分；在将该形式化模型映射至工业实践时，必须面对以下三项源于有界理性的结构性现实张力：
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{视界截断效应（Truncation Dilemma）：}有限时间窗 $\tau=\{0,\ldots,H\}$ 意味着所有超出 $H$ 的未来损益均被压缩于终止残值 $\zeta^H v_H$ 中。若评估主体人为选择较短的时间窗（例如仅对应单次迭代或短期交付节点），且在估算 $v_H$ 时忽略了遗留系统状态 $w_H$ 所隐含的高额恢复成本，则最优解 $\pi^*$ 极易诱发对当期防护投入 $\kappa_t$ 的过度削减，从而将高额的潜在不可逆损失 $I(w_H)$ 转嫁至后续周期。这一机制在数学上解释了技术债务与架构退化的非理性累积动因。
    \item \textbf{环境非平稳性与行为依赖漂移（Non-stationarity）：}模型假定转移核 $T$ 和扰动集 $\mathcal D$ 在时间窗内已预先界定。但在超长生命周期系统中，底层环境、外部依赖与上下游协议会随时间自发漂移；正如海勒姆定律（Hyrum's Law \cite{winters2020}）所指出的，随着观察者数量与系统存活时间增加，所有可观测的系统实现细节最终都会被调用方隐式依赖。这种非平稳漂移会导致即便代码文本未被修改，其外部不可逆性 $I$ 与控制缺口 $\gamma_Q$ 也会随时间推移而内生上升。
    \item \textbf{执行主体不连续性（Subject Discontinuity）：}当项目跨越较长周期后，执行主体 $s_t$ 常面临人员更替与团队重组（$s_t \neq s_{t+k}$）。若缺乏外部证据保留，历史决策信息 $h_t$ 无法被后继主体完整继承，导致认知缺口 $k_Q$ 在交接断点出现台阶式跃升。这直接决定了知识外部化（P12）、契约显式化（P5）与证据保留（P6）在长周期工程中的必要性。
\end{enumerate}
\end{remark}
...
Parnas 的信息隐藏模块划分准则与 SICP 的最少承诺原则分别对应原则 P4 与 P2 \cite{parnas1972,sicp1996}。Boehm 在《软件工程经济学》中开创性地将全生命周期资源分析与定量成本估计模型引入软件开发 \cite{boehm1981}；Ward Cunningham 最初提出的技术债务隐喻则强调“借贷以加快短期交付，未来持续偿还利息”\cite{cunningham1992}。技术债务在 KRCIGE 中可形式化为延后决策或结构妥协带来的系统内生变更成本：
\[
C_{\mathrm{debt}}
=
C_{\mathrm{deferred}}
